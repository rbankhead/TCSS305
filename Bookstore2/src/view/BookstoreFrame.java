 ...
 * 
 * @author richardbankhead (small updates for PA#03B)
 * @version Autumn 2019
 */
public final class BookstoreFrame extends JFrame {
    ...
    private void setupGUI() {
        ...
        add(makeTotalPanel(), BorderLayout.SOUTH); //RB
        
        Collections.sort(myCampusInventories.get(myCurrentCampus));
        myItemsPanel = makeItemsPanel(myCampusInventories.get(myCurrentCampus)); 
        add(myItemsPanel, BorderLayout.CENTER);
        
        add(makeCheckBoxPanel(), BorderLayout.NORTH); //RB
     ...
     * 1.e - moved clear button into total panel - Richard Bankhead
    ...
    private JPanel makeTotalPanel() {
        ...
        p.add(clearButton, BorderLayout.LINE_END);   
     ...
     * Richard Bankhead - added sort methods to display the items in the item panel in an order based on price, which changes from ascending to descending depending on the bookstore chosen
     * the sort call is object oriented for bothell but a lambda expression for Tacoma per instructions
    ...
    private JPanel makeItemsPanel(final List<Item> theItems) {
        ...
        if (myCurrentCampus.equalsIgnoreCase("bothell bookstore")) {
            Collections.sort(theItems, new Item("", BigDecimal.ZERO)); //for bothell lists items from lowest price to highest price
        } else {
            Collections.sort(theItems, (first, second) -> second.compare(second, first)); //for tacoma, lists items from highest price to lowest price
        }
     ...
     * 1.d - added campuspanel call, added BorderLayout to p - Richard Bankhead
    ...
    private JPanel makeCheckBoxPanel() {
        ...
        p.add(makeCampusPanel(), BorderLayout.WEST);
